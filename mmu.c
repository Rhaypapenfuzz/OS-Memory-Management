...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
    ...
    node_t *curr_node = freelist->head;
    node_t *prev_node = NULL; 
		// Iterates over cuurent node until we satisfy the block size condition
    while(curr_node){
      if (curr_node->blk->end - curr_node->blk->start >= blocksize){
        blk_node = curr_node; 
        break;
      } 
      prev_node = curr_node;
      curr_node = curr_node->next;
		...
		// Return error if blk node check fails
    if (!blk_node){
    ...
    // function that adds the blk to the ALLOC_LIST in ascending order based on its address
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
    ...
    // Add the blk back to the FREE_LIST based on the policy. 
    if (policy == 1){
