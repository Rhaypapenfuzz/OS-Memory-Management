...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
   ...
   */
    ...
    if(l->head == NULL){
      ...
      prevNode = currentNode = l->head;
      if(currentNode->next == NULL) { 
         if(newblk->start <= currentNode->blk->start) {  
            newNode->next = l->head;
            l->head = newNode;
         }
         else {  
         ...
         if(newNode->blk->start <= currentNode->blk->start) {  
            newNode->next = l->head;
            l->head = newNode;
         }
         else {
            while(currentNode != NULL && newNode->blk->start >= currentNode->blk->start) {
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
   ...
   */
    ...
    if(l->head == NULL){
      ...
      prevNode = currentNode = l->head;
      curr_blocksize = currentNode->blk->end - currentNode->blk->start + 1;
			// Checck for solitary node in linked list
      if(currentNode->next == NULL) {  
         if(newblk_size >= curr_blocksize) {  
			...
			// Check for multiple nodes in linked list
      else {  
         if(newblk_size >= curr_blocksize) {  
            newNode->next = l->head;
            l->head = newNode;
            ...
            while(currentNode != NULL && newblk_size <= curr_blocksize) {
                 prevNode = currentNode;
                 currentNode = currentNode->next;
                 if(currentNode != NULL)  
                       curr_blocksize = currentNode->blk->end - currentNode->blk->start;
            }
            prevNode->next = newNode;
            newNode->next = currentNode;
         }
